...
int ilog2(int x)
{
    int logOfX = 0;
    logOfX = (!!(x >> 16)) << 4;
    logOfX = logOfX + ((!!(x >> (logOfX + 8))) << 3);
    logOfX = logOfX + ((!!(x >> (logOfX + 4))) << 2);
    logOfX = logOfX + ((!!(x >> (logOfX + 2))) << 1);
    logOfX = logOfX + (!!(x >> (logOfX + 1)));
    return logOfX;
}
/*
 ...
 *   Rating: 4
 */
unsigned float_i2f(int x) { return x; }
/*
